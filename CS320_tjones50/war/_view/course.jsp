...
<body>
		...
		<c:when test="${!empty email}">
			<form action="${pageContext.request.contextPath}/giveAdvice">
				...
				<input name="courseName" type="hidden" value="${course.name}" /> <input
					name="departmentName" type="hidden" value="${department.name}" />
				<input name="adviceId" type="hidden" value="${advice.adviceId}" />
				<input name="flags" type="hidden" value="${advice.flags}" /> <input
					type="hidden" name="from"
					value="${pageContext.request.requestURI}/course">
			</form>
		</c:when>
		<c:otherwise>
			<button type="submit" class="giveAdviceFade">Give Advice</button>
			<div class="giveAdviceMessage" style="color: #FF0000;">Login to
				leave and flag advice</div>
		</c:otherwise>
	</c:choose>
	<h1>Here is the Advice for ${course.department.name} -
		${course.name}!</h1>
	<div class="ratings">
		<p2> Average grade: <fmt:formatNumber type="number"
			maxFractionDigits="2" value="${course.aveGrade}" /> out of 4.0 </p2>
		<p2> Average difficulty: <fmt:formatNumber type="number"
			maxFractionDigits="2" value="${course.aveRatings.difficulty}" /> out
		of 10 </p2>
		<p2> Average of instruction quality: <fmt:formatNumber
			type="number" maxFractionDigits="2"
			value="${course.aveRatings.instruction}" /> out of 10 </p2>
		<p2> Average supply cost: <fmt:formatNumber type="number"
			maxFractionDigits="2" value="${course.aveRatings.suppliesCost}" />
		out of 10 </p2>
		<p2> Average enjoyment: <fmt:formatNumber type="number"
			maxFractionDigits="2" value="${course.aveRatings.enjoyment}" /> out
		of 10 </p2>
	</div>
	<!-- "ratings" -->
	<br>
	...
	<form action="${pageContext.servletContext.contextPath}/course"
		...
		<div class="select-box">
			...
			<button name="sortAdvice" type="submit" class="sort">Sort
				Advice</button>
			<input name="courseName" type="hidden" value="${course.name}" /> <input
				name="departmentName" type="hidden" value="${department.name}" /> <input
				name="flag" type="hidden" value="false" />
		</div>
	</form>
	</p3>
	<c:forEach items="${course.arrAdvice}" var="advice">
		<tr>
			<td><c:if test="${advice.approved}">
					<c:if test="${advice.userActivated}">
						<p1> Course was taken ${advice.semester} of
						${advice.classYear} <br>
						<br>
						...
						Enjoyment was ${advice.adviceRating.enjoyment} <c:if
							test="${!empty email}">
							<form action="${pageContext.servletContext.contextPath}/course"
								method="post">
								<button type="submit" class="flags">Flag Inappropriate
									Advice</button>
								<input name="courseName" type="hidden" value="${course.name}" />
								<input name="departmentName" type="hidden"
									value="${department.name}" /> <input name="adviceId"
									type="hidden" value="${advice.adviceId}" /> <input name="flag"
									type="hidden" value="true" /> <input name="sort" type="hidden"
									value="Newest" />
							</form>
							<div id="flag_content">${advice.flags} flagged this advice
							</div>
							<!--flag_content -->
						</c:if> </p1>
					</c:if>
